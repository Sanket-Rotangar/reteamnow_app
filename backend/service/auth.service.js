...
export const loginUserAdminService = async ({ email, password }) => {
  ...
  // Check if user is locked due to compliance test failures (applies only to regular users, not admins)
  if (user.complianceAttempts && user.role !== 'superadmin') {
    const hasLockedTest = Array.from(user.complianceAttempts.values()).some(attempt => attempt.isLocked === true);
    if (hasLockedTest) {
      throw new Error("Your account has been locked due to compliance test failures. Please contact HR for assistance.");
    }
  }
