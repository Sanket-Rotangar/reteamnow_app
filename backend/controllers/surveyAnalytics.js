...
const getSurveyAnalytics = async (req, res) => {
  ...
  const { branch } = req.query; // Get branch filter from query params
  console.log(`Fetching analytics for survey ID: ${sid}, type: ${type || 'overview'}, branch: ${branch || 'all'}`);

  try {
    // Base analytics query with branch filtering
    const baseAnalytics = await getBaseAnalytics(sid, branch);
...
const getBaseAnalytics = async (sid, branch = null) => {
  // Import User model for branch filtering
  ...
  // If branch filter is specified, get list of employee IDs for that branch
  let branchEmployeeIds = null;
  if (branch) {
    console.log(`Filtering analytics by branch: ${branch}`);
    
    // Get all users from the specified branch
    const usersInBranch = await User.find({ branch }).select('_id username');
    const adminsInBranch = await Admin.find({ branch }).select('_id username');
    
    // Create arrays of both ObjectIds and usernames since empId could be either
    const userIds = usersInBranch.map(user => user._id.toString());
    const usernames = usersInBranch.map(user => user.username);
    const adminIds = adminsInBranch.map(admin => admin._id.toString());
    const adminUsernames = adminsInBranch.map(admin => admin.username);
    
    branchEmployeeIds = [...userIds, ...usernames, ...adminIds, ...adminUsernames];
    
    console.log(`Found ${branchEmployeeIds.length} employees in branch ${branch}`);
    
    if (branchEmployeeIds.length === 0) {
      // No employees in this branch, return empty analytics
      return {
        sid,
  ...
  // Build the aggregation pipeline with optional branch filtering
  const matchStage = { sid };
  
  // Modified aggregate pipeline to handle different question types, skipped questions, and branch filtering
  const pipeline = [
    { $match: matchStage },
    { $unwind: "$responses" }
  ];
  ...
  if (branchEmployeeIds) {
    pipeline.push({
      $match: {
        "responses.empId": { $in: branchEmployeeIds }
      }
    });
  }
  ...
  const survey = await SurveyResponse.aggregate(pipeline);

  // Get total participants and skipped questions count with branch filtering
  const participantPipeline = [
    { $match: matchStage }
  ];
  
  participantPipeline.push(
    {
      $project: {
        filteredResponses: branchEmployeeIds ? {
          $filter: {
            input: "$responses",
            as: "response",
            cond: { $in: ["$$response.empId", branchEmployeeIds] }
          }
        } : "$responses"
      }
    },
    {
      $project: {
        participantCount: { $size: "$filteredResponses" },
        skippedCount: {
          $size: {
            $filter: {
              input: {
  ...
  const participantStats = await SurveyResponse.aggregate(participantPipeline);
