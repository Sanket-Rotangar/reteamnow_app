...
export const getAllHiringRequests = async (req, res) => {
  ...
  try {
    ...
    if (userJobRole !== 'superadmin') {
      ...
      const orConditions = [{ jobRole: userJobRole }];

      // THE FIX IS HERE
      // Only add the userId condition if it exists and is not the string "undefined"
      if (userId && userId !== 'undefined') {
        orConditions.push({ assignedToUserId: userId });
      }

      query = { $or: orConditions };
    }
    // If userId is "undefined", this condition prevents the 'assignedToUserId'
    // field from being added to the query, thus avoiding a crash.
    const hiringRequests = await HiringRequest.find(query).sort({ createdAt: -1 });
    res.json(hiringRequests);
  } catch (err) {
    console.error("Error fetching hiring requests:", err);
